...
const jump_velocity: rl.c.Vector3 = .{ .x = 0, .y = 5, .z = 0 };
const gravity: rl.c.Vector3 = .{ .x = 0, .y = -10, .z = 0 };
...
pub fn update(playerPosition: *rl.c.Vector3, angView: rl.c.Vector3) void {
    ...
    if (rl.c.IsKeyPressed(rl.c.KEY_SPACE)) {
    ...
    if (playerPosition.y > 2) velocity = gmath.add(velocity, gmath.mul(gravity, rl.c.GetFrameTime())) else {
        playerPosition.y = 2;
        velocity.y = 0;
    }
}
